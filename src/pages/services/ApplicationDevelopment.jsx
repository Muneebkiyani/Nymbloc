                    ...
                    <div className="pricing-grid">
                        {plans.map((plan, index) => (
                            <div key={index} className={`pricing-card ${plan.recommended ? 'recommended' : ''} ${plan.colorClass}`} data-aos="fade-up" data-aos-delay={index * 100}>
                                ...
                                <Link to={`/checkout?service=Application&plan=${plan.name}`} className={`btn ${plan.recommended ? 'btn-primary' : 'btn-outline'}`} style={{ width: '100%', borderColor: plan.recommended ? 'transparent' : 'rgba(255,255,255,0.2)' }}>
                                    Select Plan
                                </Link>
                            </div>
                        ))}
